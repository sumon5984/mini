...
const connect = async (file_path) => {
  ...
  try {
    ...
    if (!sessionDir) {
      console.error(`❌ Invalid session path for file_path:`, file_path);
    ...
    conn.ev.on(
      ...
      async ({ connection, lastDisconnect, isNewLogin }) => {
        if (connection === "close") {
          ...
          if (isNewLogin) console.log(`🆕 [${file_path}] New login detected`);
...
class ConnectionManager extends EventEmitter {
  constructor() {
    super();
    this.connections = new Map(); // active connections (per user/file_path)
    this.connecting = new Set(); // currently connecting sessions
    this.reconnectAttempts = new Map(); // track reconnect attempts per user
    this.scheduled = new Map(); // track timers per user
    this.MAX_RECONNECT = 5;
  ...
  getReconnectDelay(id) {
    const attempt = Math.max(this.reconnectAttempts.get(id) || 1, 1);
    const delay = Math.min(
  ...
  scheduleReconnect(id, fn) {
    if (this.scheduled.has(id)) return; // already scheduled

    if (!this.canReconnect(id)) {
      console.log(`❌ [${id}] Max reconnect attempts reached. Stopping.`);
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        console.error(`❌ Reconnect failed [${id}]:`, err?.message || err);
        this.removeConnecting(id);
      }
    }, delay);
    ...
    console.log(
      `🔄 [${id}] Scheduled reconnect (attempt ${attempt}/${this.MAX_RECONNECT}) in ${delay}ms`
    );
  }
...
const connect = async (file_path) => {
  ...
  try {
    ...
    if (!(await fs.pathExists(credsPath))) {
      manager.removeConnecting(file_path);
      throw new Error(
        `❌ [${file_path}] Credentials not found at: ${credsPath}`
      );
    }
    ...
    conn.ev.on(
      ...
      async ({ connection, lastDisconnect, isNewLogin }) => {
        if (connection === "close") {
          ...
          console.log(
            `🛑 [${file_path}] Closed: ${statusCode || "undefined"} - ${
              errorMsg || "unknown"
            }`
          );
          ...
          if (manager.shouldDelete(statusCode)) {
            console.log(
              `🗑️ [${file_path}] Critical error, deleting session...`
            );
            manager.removeConnection(file_path);
